class Pet
  ...
  def food_cost(days)
    FOOD_COST_PER_KG[animal_type_id] * food_consumed_per_day * days
  end
  ...
  def self.cost_to_feed(pets, days)
    total_cost = 0
    pets.each { |pet| total_cost += pet.food_cost(days) }
    total_cost
  end
  ...
  def self.group_by_animal_type(pets)
    grouped_pets = Hash.new { |hash, key| hash[key] = [] }
    pets.each { |pet| grouped_pets[pet.animal_type_id] << pet.name }
    grouped_pets
  end
end